...
describe('ChatWindow layout', () => {
  it('exposes viewer request and host moderation controls', () => {
    ...
    expect(componentSource).not.toContain('主畫面')
  ...
  it('exposes stage actions while leaving their controls to the live room header', () => {
    expect(componentSource).toContain('stageGuests: computed')
    expect(componentSource).toContain('removeGuest,')
    expect(componentSource).toContain('leaveStage,')
    expect(componentSource).not.toContain('data-testid="participant-leave-stage"')
  })
})
